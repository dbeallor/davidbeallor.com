...
function preload(){
	for (var i = 0; i < 4; i++)
		help_images[i] = loadImage("help" + i + ".png");
...
function setup() {
	...
	button_labels = ["CONTINUE", "UNDO", "FRACTALIZE", "SKIP\nEDGE", "HIDE\nEDGE", "SAVE", "LOAD", "COLOR\nSCHEME", "GRID\nLINES", "RESTART", "HELP"];
...
function mousePressed(){
	if (mouseButton === LEFT){
		if (noOpenWindows()){
			...
			if (withinBounds(mouseX, mouseY, control_box.bounds) && !fractalize){
		...
		// Sample seed menu mouse events
			...
			for (var i = 0; i < file_browser.files.length; i++){
				if (withinBounds(mouseX, mouseY, file_browser.files[i].bounds)){
					loading_sample_seed = false;
					loadStrings(file_browser.files[i].name, loadSeed);
				}
			}
		}
